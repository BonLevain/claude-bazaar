...
export class BuildCommand {
  ...
  private async generateDockerfile(projectDir: string, config: ShipyardConfig): Promise<string> {
    ...
    // Copy runtime files into .shipyard to avoid symlink issues
...
COPY .shipyard/runtime/package.json ./
RUN npm install --production
COPY .shipyard/runtime/dist ./dist
...
`;
    ...
    console.log('Generated .shipyard/Dockerfile');
  ...
  private async copyRuntimeFiles(projectDir: string, shipyardDir: string): Promise<void> {
    ...
    console.log('Copied runtime files to .shipyard/runtime');
  }
